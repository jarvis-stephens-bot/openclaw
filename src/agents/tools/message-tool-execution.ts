...
import {
  ...
  projectMessageActionPartialDelivery,
      ...
      return await withChannelReadAuthority(
        ...
        async () => {
          ...
          try {
            ...
            const partialDelivery = messageActionAuthorization.scheduled
              ? projectMessageActionPartialDelivery(error)
              : undefined;
            if (partialDelivery && action !== "broadcast") {
              const target =
                readToolStringParam(params, "to") ??
                readToolStringParam(params, "target") ??
                "unknown";
              const channel =
                ...
                normalizeMessageChannel(
                  typeof scope.channel === "string" ? scope.channel : undefined,
                ) ??
                "unknown";
              result =
                action === "send"
