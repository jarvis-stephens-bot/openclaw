...
The fallback [startup corpus step](/ci/pipeline) admits runs from the runner's available CPU count, keeping all four state shards and the config corpus within the existing no-output budget.
