...
export async function runAgentsApiAttempt(
  ...
  assertCurrent();
  if (!params.resolvedApiKey) { throw new Error("Agents API MVP requires an OpenAI API key"); }
  ...
  const submit = (text: string) => {
    assertCurrent();
    if (!remoteSessionId || stopped) { throw new Error("Agents API turn is stopped"); }
    const sessionId = remoteSessionId;
    submission = submission.then(() => client.message(sessionId, text, AbortSignal.timeout(60_000)));
  ...
  const stop = (requested = true) => {
    ...
    if (remoteSessionId && submitted) {
      ...
      cancellation = (async () => {
        ...
        try { await admittedSubmission; } catch (error) { submissionError = error; }
  ...
  const handle = {
    ...
    queueMessage: async (text: string, options?: Parameters<Parameters<typeof setActiveEmbeddedRun>[1]["queueMessage"]>[1]) => {
      assertCurrent();
      if (stopped || !remoteSessionId || !submitted) { throw new Error("Agents API turn is not ready for steering"); }
      if (options?.images?.length) { throw new Error("Agents API MVP accepts text steering only"); }
  ...
  try {
    ...
    try {
      ...
      for await (const event of events) {
        ...
        if (event.type === "agent.session.turn.output_text.delta" || event.type === "agent.session.turn.output_text.done") {
          ...
          texts.set(key, event.type.endsWith(".done") ? event.text ?? "" : (texts.get(key) ?? "") + (event.delta ?? ""));
          emitAgentEvent({ runId: params.runId, stream: "assistant", data: { text: [...texts.values()].join("\n"), delta: event.delta ?? "" } });
        ...
        if (event.type.startsWith("agent.session.turn.") && event.turn?.subagent_id === null &&
          ...
          rootTurn = event.turn;
          if (event.type.endsWith(".failed")) { throw new Error(event.turn.error?.message ?? "Agents API turn failed"); }
          if (event.type.endsWith(".cancelled")) { terminal = { kind: "aborted", source: "runtime" }; }
          break;
        }
      }
    } finally {
    ...
    if (!rootTurn) { throw new Error("Agents API stream closed before the root turn settled; reset or inspect the session before retrying"); }
    if (terminal.kind === "ok") {
      const items = await client.items(remoteSessionId, rootTurn.id, controller.signal);
      assertCurrent();
      const completedMessages = items.filter((item) => item.type === "message" && item.role === "assistant" && item.status === "completed");
      const finalItems = completedMessages.filter((item) => item.phase === "final_answer");
      const visibleItems = finalItems.length ? finalItems : completedMessages.filter((item) => item.phase !== "commentary");
      const text = visibleItems.map((item) => item.content?.filter((part) => part.type === "output_text").map((part) => part.text ?? "").join("") ?? "").join("\n");
      const nativeUsage = rootTurn.usage;
      if (nativeUsage) {
        usage = { ...usage, input: nativeUsage.input_tokens - (nativeUsage.input_tokens_details?.cached_tokens ?? 0), output: nativeUsage.output_tokens, cacheRead: nativeUsage.input_tokens_details?.cached_tokens ?? 0, totalTokens: nativeUsage.input_tokens + nativeUsage.output_tokens };
        params.hostCapabilities.reportOutputTokens?.(usage.output);
        calculateCost(params.model, usage);
      }
      if (text) {
        const assistant: AssistantMessage & { idempotencyKey: string } = { role: "assistant", content: [{ type: "text", text }], api: "openai-responses", provider: "openai", model: "gpt-6-astra", usage, stopReason: "stop", timestamp: Date.now(), idempotencyKey: `agentsapi:${remoteSessionId}:${rootTurn.id}` };
        const append = await appendSessionTranscriptMessageByIdentityStrict({
          ...params.sessionTarget,
          config: params.config,
          message: assistant,
          prepareMessageAfterIdempotencyCheck: (message) => { assertCurrent(); return message; },
        });
        assertCurrent();
        if (append.kind !== "result") { throw new Error("Agents API assistant transcript append was refused"); }
        lastAssistant = append.result.message;
      }
    }
  } catch (error) {
    if (!stopped && submitted && !rootTurn) { stop(false); }
    terminal = timedOut ? { kind: "timeout", phase: "prompt", source: "runtime", aborted: true }
      : params.abortSignal?.aborted ? { kind: "aborted", source: "external" }
      : interrupted ? { kind: "aborted", source: "runtime" }
      : { kind: "failed", source: "prompt", error };
  } finally {
    clearTimeout(timer);
    params.abortSignal?.removeEventListener("abort", onAbort);
    try { await cancellation; } catch (error) { terminal = { kind: "failed", source: "prompt", error }; }
    stopped = true;
    clearActiveEmbeddedRun(params.sessionId, handle, params.sessionKey, params.sessionFile);
  }
  const assistantTexts = lastAssistant?.content.filter((part) => part.type === "text").map((part) => part.text) ?? [];
  return {
    terminal, sessionIdUsed: params.sessionId, sessionFileUsed: params.sessionFile, agentHarnessId: "agentsapi",
    messagesSnapshot: SessionManager.open(params.sessionTarget, params.workspaceDir).buildSessionContext().messages,
    assistantTexts, lastAssistant, currentAttemptAssistant: lastAssistant, currentAttemptCompletedAssistant: lastAssistant,
    assistantTranscriptOwned: Boolean(lastAssistant),
    assistantTranscriptIdempotencyKey: lastAssistant && rootTurn ? `agentsapi:${remoteSessionId}:${rootTurn.id}` : undefined,
    toolMetas: [], didSendViaMessagingTool: false, messagingToolSentTexts: [], messagingToolSentMediaUrls: [], messagingToolSentTargets: [],
    cloudCodeAssistFormatError: false, attemptUsage: usage,
